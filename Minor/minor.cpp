...
bool lw(arrayOfRegister* reg,Memory* m, int x1, int x2, int x3){
    ...
    if(x1>0 && x1<32 && x3>=0 && x3<32) {
        ...
        if ((loc % 4) != 0){
            ...
            CC++;
            cout<< "cycle " << CC << ":\t" << "DRAM request issued" << endl;
            int pre = CC + 1;
            string yy;
            CURR = loc/1024;
            ...
            cout << "cycle " << pre << "-" << CC << ":\t" << "$" << x1 << " = " <<  m->read(loc) <<"; " << yy << endl;
        }
    }
    else {
...
bool sw(arrayOfRegister* reg,Memory* m, int x1, int x2, int x3){
    ...
    if(x1>0 && x1<31 && x3>=0 && x3<32) {
        ...
        if ((x2 % 4) != 0){
            ...
            PREV=CURR;
            m->set(loc, x2);
            cout<< "cycle " << pre << "-" << CC << ":\t" << "Memory address " << x2 << "-" << x2+3 << " = " << loc << "; " << yy << endl;
        }
    }
    else {
...
void execute(arrayOfRegister* regs, Memory* m, map<int,ins*> instructions){
    ...
    for(int i = 0; i < pcs.size(); i++) {
        ...
        if(k==-1) {
            cout<<"Instruction: "+instructions.find(pcs[i])->second->sen+"\n";
            return;
        }
